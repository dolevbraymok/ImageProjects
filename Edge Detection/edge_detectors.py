...
import sys
import os
sys.path.append(os.path.abspath(os.path.join('..', 'Filters')))
# Now do your import
import Filters.filters as filters

SOBELX_KERNEL = np.array(([-1,0,1],[-2,0,2],[-1,0,1]),dtype='int')
SOBELY_KERNEL = np.transpose(SOBELX_KERNEL)
...
def canny_edge_detector(image):
    ...
    """VALUES NEED TO CHANGE BY EXPIRMENT FOR BEST RESULTS"""
    sigma = 5
    high_threshold = 200
    low_threshold = 100
    ...
    gx = filters.convolve(blurred,SOBELX_KERNEL)
    gy = filters.convolve(blurred,SOBELY_KERNEL)
